...
void SSD1780_init_HW(void)
{
    ...
    /*SSD1780_SET_MULTIPLEX_RATIO*/
    /* When you set "multiplex ratio" of the display's driver
     * (whatever it means) to small values - the display
     * is refreshed more often and the brightness becomes higher.
     * In practice it limits the maximum image height (sets it directly in pixels).
     * */
    SSD1780_send_cmd_with_val(SSD1780_SET_MULTIPLEX_RATIO, 0x3F); // 0xA8, default: 0x3F, gives 8 pages

    SSD1780_send_cmd_with_val(SSD1780_SET_VERTICAL_OFFSET, 0x00);    // 0xD3 No offset
    SSD1780_send_cmd_with_val(SSD1780_SET_CLOCK_DIV_RATIO, 0xf0);    // 0xD5 Set divide ratio
    SSD1780_send_cmd_with_val(SSD1780_SET_PRE_CHARGE_PERIOD, 0x22);  // 0xD9
    SSD1780_send_cmd_with_val(SSD1780_SET_COM_PINS_HW_CONFIG, 0x02); // 0xDA
    SSD1780_send_cmd_with_val(SSD1780_SET_COMH_DESELECT_LEVEL, 0x49); // 0xDB 0.83*vref
    SSD1780_send_cmd_with_val(SSD1780_SET_CHARGE_PUMP, SSD1780_CHARGE_PUMP_ENABLED); // 0x8D
...
{
    Uint8 packet[2];
    packet[0] = SSD1780_SEND_COMMAND_TOKEN;  // 0x00 D/#C low = it's a command
    packet[1] = command_token;               // Command
    return USBSTK5515_I2C_write(OSD9616_I2C_ADDR, packet, 2);
}

Int16 SSD1780_send_cmd_with_val(Uint8 command_token, Uint8 val)
{
    Uint8 packet[3];
    packet[0] = SSD1780_SEND_COMMAND_TOKEN;  // 0x00 D/#C low = it's a command
    packet[1] = command_token;         // Command
...
{
    Uint8 packet[2];
    packet[0] = SSD1780_SEND_DATA_TOKEN;  // 0x40 D/#C high = it's a data
...
Int16 SSD1780_set_VHAM_page_addres(Uint8 start_addres_3b, Uint8 end_addres_3b)
{
    ...
    Uint8 packet[4];
    packet[0] = SSD1780_SEND_COMMAND_TOKEN;
                                                  ...
                                                  Uint8 V_step_size)
{
    ...
    packet[4] = (Uint8) interval;
    packet[5] = end_page & 0x7;      // last page which the setup is addressed to
    packet[6] = V_step_size;
    USBSTK5515_I2C_write(OSD9616_I2C_ADDR, packet, 7);
}

Int16 SSD1780_set_memory_addressing(SSD1780_mem_mode_e mode_2bit)
{
    return SSD1780_send_cmd_with_val(SSD1780_SET_MEM_ADDRESSING_MODE, mode_2bit & 0x3); //0x20
}

void SSD1780_set_vertical_offset(Uint8 v_px_pos_6bit)
{
    v_px_pos_6bit &= 0x3F; // apply 6 bit mask
    SSD1780_send_cmd_with_val(SSD1780_SET_VERTICAL_OFFSET, v_px_pos_6bit);  // 0xD3
}
...
void SSD1780_set_vertical_scroll_area(Uint8 fixed_part_height, Uint8 floating_part_height)
{
    Uint8 packet[4];
    /* Keep first 8 rows from vertical scrolling  */
    packet[0] = SSD1780_SEND_COMMAND_TOKEN;
    packet[1] = SSD1780_SET_V_SCROLL_AREA; // Set Vertical Scroll Area
    packet[2] = fixed_part_height & 0x3F;    // Set No. of rows in top fixed area
    packet[3] = floating_part_height & 0x7F; // Set No. of rows in scroll area
...
Int16 SSD1780_clear_display_ram(void)
{
    ...
    for (page = 0; page < SSD1780_PAGE_COUNT; page++)
    {
        ...
        {
            ret |= SSD1780_send_single_data(0x00);
        }
